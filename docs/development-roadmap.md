...
| Governance and model routing | Complete for local/source gates | Provider-neutral task-stage routing; mandatory pre-dispatch locality/classification, approved HTTPS endpoint, known versioned pricing, durable intent/reservation, and pre-dispatch-only fallback; bounded MCP/model data; exact/estimated usage; typed retry/reconciliation; and terminal stop closures. Pricing configuration rejects missing, malformed, negative, and non-finite rates. Numeric budgets are optional limits, while empty pricing intentionally denies dispatch and estimates are not invoices. Final local verification: `lab-agent` 406/406, `canvus-mcp` 37/37, governance matrix 131/131 across four runs without flakes, endpoint suite 15/15, reviewer cycle 3 9.7/10 SEALED. Organization-approved endpoint/classification matrix, price maintenance, invoice reconciliation, and live SDK/API checks remain operational gates. |
...
| Phase 7 validation and approval gates | Complete for local/source infrastructure (2026-07-23) | Typed canonical `litl-canonical-json-v1` SHA-256 proposal/result hashes; deterministic structural dry-run validation, default-on but disable-able only for new scheduling, visibly not scientific; canvas-scoped append-only validation/approval evidence; credential-verified scientist then lab-lead gates; Browser projections only. Existing evidence remains durable when scheduling is disabled. |
| Execution modes, terminal stops, and SMTP outbox | Complete for local/source gates (2026-07-23) | Mode parsing, fail-closed connected unknown modes, gate-safe staged multi-round continuation, terminal events, and durable SMTP outbox are locally verified. Structured-output emissions reject non-object payloads and log schema failures as metadata only; the durable demo seeder accepts `{idea+auto: ...}` alongside `{idea: ...}`. The loop remains mock/dry-run only, while real identity, scientific validation, hardware, and external SMTP operations remain gates. |
...
- Clarified the operating contract: pricing remains mandatory even when numeric budgets are omitted; an empty pricing table intentionally denies dispatch; pricing estimates are not invoices; and local OpenAI-compatible deployments need an approved HTTPS/TLS endpoint. These controls have no bypass flag.
...
- `in_silico_validation_enabled` defaults true and schedules new deterministic structural validation; disabling it skips only that new scheduling, not durable evidence, approval reconciliation, loops, or independent execution gates. The deterministic result is not scientific simulation or measured evidence.

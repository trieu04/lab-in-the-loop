...
4. Validate the emitted setup before any write: evidence must be `sufficient`, citations must resolve to the current ledger, and deterministic full-boundary acronym scanning covers the original idea text, emitted setup fields, and all bounded evidence excerpts. An unknown alphabetic initialism clears only through exactly one strict setup-local `long form (ACRONYM)` definition or one unique approved-dictionary entry; conflicting, cross-field, evidence-only, or idea-only definitions do not clear it. Explicit flags are acronym-like when uppercase, compact mixed-case, or present case-insensitively in the approved dictionary; unknown all-lowercase words/compounds remain advisory. A non-acronym flag blocks only with a concrete material impact and at least two distinct alternatives.
5. If executable, create `[EXP:Setup v001]` as a generated Browser artifact backed by `ArtifactStore` and connect idea → setup.
6. If evidence is insufficient, a material ambiguity remains, or an acronym is unresolved, create one deduplicated `[EXP:Needs Input]` Browser artifact instead; invalid citations write nothing and stay retryable.
...
- The setup prompt asks the model to expand inferable abbreviations on first use, operationalize vague readouts (for example, `signal`), and record conservative, reversible defaults when omitted detail does not materially affect the experiment. These are model-facing quality instructions, not a separate deterministic semantic validator. Glossary-like non-acronym flags are advisory and do not create `[EXP:Needs Input]`; a non-acronym flag blocks only with a concrete material impact and at least two distinct alternatives. `ambiguity_flags` are therefore reserved for unresolved choices that materially affect safety, feasibility, resources, experimental design, or interpretation.
- Do not invent domain-specific meanings for ambiguous acronyms. Deterministic full-boundary scanning checks the original idea, emitted setup fields, and every bounded retrieved evidence excerpt. An unknown alphabetic initialism clears only through exactly one strict setup-local `long form (ACRONYM)` definition within a single setup field/list item or a unique approved-dictionary entry; conflicting, cross-field, idea-only, and evidence-only definitions do not clear it. Explicit uppercase and compact mixed-case flags remain acronym blockers, while case-insensitive dictionary lookup governs lowercase known terms. Unknown all-lowercase words and compounds are not guessed to be acronyms and remain advisory unless the normal setup-boundary scan independently detects an acronym token.
- If evidence is insufficient, a material ambiguity remains, or an acronym is unresolved, create an explicit Needs Input artifact; do not silently lower confidence and proceed. Evidence-sufficiency and ledger-citation validation are unchanged and fail closed.
...
| Material domain ambiguity or unresolved acronym | a non-acronym blocks only with a concrete material impact and at least two distinct alternatives; the deterministic acronym scan covers idea + setup + evidence excerpts. Either blocking case writes Needs Input; no guessed expansion is accepted. |

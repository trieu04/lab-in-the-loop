...
- Refined setup-generation instructions to expand inferable abbreviations on first use, operationalize vague readouts such as `signal`, and record conservative, reversible defaults when an omitted detail does not materially affect the experiment. These remain model-facing quality instructions; glossary-like non-acronym flags no longer create `[EXP:Needs Input]` artifacts. A non-acronym flag blocks only when it supplies a material impact and at least two distinct alternatives; `ambiguity_flags` therefore describe unresolved choices that materially affect safety, feasibility, resources, experimental design, or interpretation.
- The grounding gate retains deterministic full-boundary acronym scanning across the original idea, emitted setup, and bounded evidence excerpts. An unknown alphabetic initialism can clear only through exactly one strict setup-local `long form (ACRONYM)` definition in a single field/list item or a unique approved-dictionary entry. Explicit uppercase, compact mixed-case, and dictionary-known case variants retain acronym handling; unknown all-lowercase words and compounds remain advisory rather than being guessed into acronym blockers.

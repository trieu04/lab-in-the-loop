...
Before dispatch, locality classifies source metadata and any retrieved `untrusted_data` envelopes. An unknown or restricted classification, missing/invalid endpoint, or absent provider allowlist entry denies the call before provider content is sent. Built-in OpenAI (`https://api.openai.com/v1`) and Claude (`https://api.anthropic.com`) endpoints are credential-gated defaults; explicit `LAB_AGENT_PROVIDER_ENDPOINTS` overrides are the endpoints passed to the SDKs. OpenAI accepts approved HTTP(S) custom endpoints, while Claude requires HTTPS; a custom OpenAI-compatible deployment must explicitly use the same URL in its endpoint approval and `LAB_AGENT_OPENAI_BASE_URL`. Unknown/custom provider names are not dynamically constructed and fail closed.
...
| Provider dispatch | Sending unapproved data, unpriced calls, duplicate/ambiguous submission | Classify before dispatch; require provider-specific endpoint scheme (OpenAI HTTP(S), other providers HTTPS) + locality allowlist + known versioned model price; durable intent, reservation, typed retry, and capability-aware reconciliation; no blind redispatch |
...
- Governance is local/source-tested, not an external authorization: operators must maintain approved provider/locality classifications, provider-specific endpoint schemes (OpenAI HTTP(S), other providers HTTPS), and versioned model prices. This repository does not verify live provider SDK/API behavior, endpoint reachability, organization approval, or invoice reconciliation.

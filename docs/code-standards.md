...
The enforcement path is `lab_agent/orchestrator_emit.py:coerce_or_fail` plus `_emit_validated`: `coerce_or_fail` first requires a JSON object, then validates it against the Pydantic schema and raises a metadata-only `SchemaValidationError` instead of coercing or defaulting missing fields. Scalars, lists, and invalid objects fail closed under fixed validation categories. `_emit_validated` logs only the stage, model name, and fixed error class before returning `None`; it must never retain or log raw model output, Pydantic input/context, or validation text that can contain output values. The setup, result, and decision callers propagate that fail-closed outcome without writing a note or connector or retrying within the same cycle. The canvas is left exactly as it was; the trigger's durable attempt is marked failed/backoff/quarantine-eligible so a later due poll can retry safely.

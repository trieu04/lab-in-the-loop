...
directly" idiom used by :mod:`lab_agent.state.attempts`/
:mod:`lab_agent.state.attempts_retry`.
"""
...
def _content_hash(payload: dict[str, Any], metadata: dict[str, Any], provenance: dict[str, Any]) -> str:
...
def _get_document(
    conn: sqlite3.Connection, *, opaque_id: str, canvas_id: str
) -> ArtifactDocument | None:
    record = artifacts_repo.get_artifact(conn, opaque_id=opaque_id)
    if record is None or record.canvas_id != canvas_id:
        return None
    version = artifacts_repo.get_version(conn, opaque_id=opaque_id, version=record.current_version)
    if version is None:  # pragma: no cover - every artifact has a version
        return None
    widget = artifact_widgets.get_mapping(conn, opaque_id=opaque_id)
...
def _list_versions(
    ...
    return [
        ...
        for item in artifacts_repo.list_versions(conn, opaque_id=opaque_id)
    ]
...
def _get_or_create_and_compose(
    ...
    *,
    ...
    version = artifacts_repo.get_version(conn, opaque_id=record.opaque_id, version=record.current_version)
    assert version is not None  # invariant: every artifact row has >=1 version
    widget = artifact_widgets.get_mapping(conn, opaque_id=record.opaque_id)
    return _compose_document(record, version, widget_id=widget.widget_id if widget else None)

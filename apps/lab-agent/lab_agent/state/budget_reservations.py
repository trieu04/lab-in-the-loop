...
def _record(row: sqlite3.Row) -> BudgetReservation:
    return BudgetReservation(
        ...
        actual_tokens=row["actual_tokens"], actual_cost_usd=row["actual_cost_usd"], status=row["status"],
    )


def totals(conn: sqlite3.Connection, *, canvas_id: str, run_id: str | None) -> tuple[int, float, int, float]:
    """Return committed then reserved totals, optionally scoped to one run."""
    where = "canvas_id=?" if run_id is None else "canvas_id=? AND run_id=?"
    args: tuple[str, ...] = (canvas_id,) if run_id is None else (canvas_id, run_id)
    row = conn.execute(
        "SELECT "
        "COALESCE(SUM(CASE WHEN status='committed' THEN actual_tokens ELSE 0 END), 0), "
        ...
        f"FROM budget_reservations WHERE {where}", args,
    ).fetchone()
...
def reserve(
    ...
    """Create one hold under an immediate transaction after rechecking both limits."""
    ...
    try:
        ...
        if row is not None:
            ...
            if (record.intent_key, record.canvas_id, record.run_id, record.estimated_tokens,
                    record.estimated_cost_usd) != (intent_key, canvas_id, run_id, tokens, cost_usd):
                raise BudgetReservationConflictError("reservation identity has incompatible inputs")
            conn.execute("COMMIT")
            return record
        by_run = totals(conn, canvas_id=canvas_id, run_id=run_id)
        by_canvas = totals(conn, canvas_id=canvas_id, run_id=None)
        ...
        now = clock()
        conn.execute(
            "INSERT INTO budget_reservations (reservation_id, intent_key, canvas_id, run_id, "
            "estimated_tokens, estimated_cost_usd, status, created_at, updated_at) "
            "VALUES (?, ?, ?, ?, ?, ?, 'reserved', ?, ?)",
            (reservation_id, intent_key, canvas_id, run_id, tokens, cost_usd, now, now),
        )
        conn.execute("COMMIT")
    except Exception:
        conn.execute("ROLLBACK")
        raise
    return get(conn, reservation_id)


def settle(
    conn: sqlite3.Connection, *, clock: Clock, reservation_id: str, action: str,
    actual_tokens: int | None = None, actual_cost_usd: float | None = None,
) -> tuple[BudgetReservation, bool]:
    """Commit or release a hold exactly once; return whether this changed it."""
    conn.execute("BEGIN IMMEDIATE")
    try:
        record = get(conn, reservation_id)
        if record.status == action:
            conn.execute("COMMIT")
            return record, False
        if record.status != "reserved":
            raise BudgetReservationConflictError(
                f"cannot {action} a {record.status} reservation"
            )
        now = clock()
        if action == "committed":
            ...
            conn.execute(
                "UPDATE budget_reservations SET status='committed', actual_tokens=?, "
                "actual_cost_usd=?, updated_at=?, settled_at=? WHERE reservation_id=?",
                (actual_tokens, actual_cost_usd, now, now, reservation_id),
            )
        else:
            conn.execute(
                "UPDATE budget_reservations SET status='released', updated_at=?, settled_at=? "
                "WHERE reservation_id=?", (now, now, reservation_id),
            )
        conn.execute("COMMIT")
    except Exception:
        conn.execute("ROLLBACK")
        raise
    return get(conn, reservation_id), True


def get(conn: sqlite3.Connection, reservation_id: str) -> BudgetReservation:
    row = conn.execute(
        "SELECT * FROM budget_reservations WHERE reservation_id=?", (reservation_id,)
    ).fetchone()
    if row is None:
